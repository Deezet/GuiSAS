...
contains
  ...
  subroutine sindeg_gaussian(x, mu, sig, gaussianval)
    double precision, intent(in) :: x, mu, sig
    double precision, intent(out) :: gaussianval
    gaussianval =&
      abs(sin(x*deg))/ (sq2pi*sig) * dexp(-0.5d0*((x-mu) / sig)**2)
  end subroutine sindeg_gaussian
  ...
  subroutine sinx(x, mu, sig, sinxval)
    double precision, intent(in) :: x, mu, sig
    double precision, intent(out) :: sinxval
    sinxval = sin(x*deg)
  end subroutine sinx
  
  subroutine cosx(x, mu, sig, cosxval)
    double precision, intent(in) :: x, mu, sig
    double precision, intent(out) :: cosxval
    cosxval = cos(x*deg)
  end subroutine cosx
  ...
  subroutine resolution_smear(q, I, sigQ, Nq, Ismear)
    ...
    integer :: iq, jq, iq_tilde, iq_low, iq_high
  ...
  subroutine resolution_smear_interpolating(q, I, sigQ, Nq, Ismear)
    ...
    double precision :: cq_running, q_low_value, q_high_value
    double precision :: qlow_steps, qhigh_steps
    if (Nq .eq. 1) then
      Ismear = I
      return
    end if
  ...
  subroutine magnetic_scattering(q, xi, sin2alpha, plus_minus, nuclear_parameters,&
      magnetic_parameters, p_nuclear, p_magnetic,&
      Nq, Np_nuc, Np_mag, ff_intensity)
    ! Calculate magnetic scattering
    ! xi: either poleff*flipp or just poleff (or just 1)
    ! sin2alpha: integral of sin(alpha)**2 for the measured angles divided by angle
    ! typical sin2alpha values: 10° sector (meaning +-5°): 0.99746538470663215
    !               20° sector: 0.98990776678064341
    !               40° sector: 0.96036270980010319
    !               60° sector: 0.91349666211001834
    !              180° sector: 0.5
    double precision, intent(in) :: q
    double precision, intent(in) :: xi, sin2alpha, plus_minus
    double precision, dimension(Np_nuc), intent(in) :: nuclear_parameters
    double precision, dimension(Np_mag), intent(in) :: magnetic_parameters
    external p_nuclear
    external p_magnetic
    integer, intent(in) :: Nq, Np_nuc, Np_mag
    double precision, intent(out) :: ff_intensity

    double precision :: f_nuclear, f_magnetic

    interface
      double precision function p_nuclear(q, p, Np)
        double precision, intent(in) :: q
        double precision, dimension(Np), intent(in) :: p
        integer, intent(in) :: Np
      end function

      double precision function p_magnetic(q, p, Np)
        double precision, intent(in) :: q
        double precision, dimension(Np), intent(in) :: p
        integer, intent(in) :: Np
      end function
    ...
    ! get nuclear and magnetic amplitudes
    f_nuclear = p_nuclear(q, nuclear_parameters, Np_nuc)
    f_magnetic = p_magnetic(q, magnetic_parameters, Np_mag)

    ff_intensity = abs(f_nuclear)**2 +&
        (abs(f_magnetic)**2 - plus_minus*2d0*xi*f_nuclear*f_magnetic)*sin2alpha
  end subroutine magnetic_scattering

  subroutine crossterm(&
    q, xi, sin2alpha, nuclear_parameters, magnetic_parameters, &
    p_nuclear, p_magnetic, Np_nuc, Np_mag, crossterm_intensity)
    double precision, intent(in) :: q
    ...
    external p_nuclear
    external p_magnetic
    integer, intent(in) :: Np_nuc, Np_mag
    double precision, intent(out) :: crossterm_intensity
    ...
    crossterm_intensity = -4d0*xi*(f_nuclear)*(f_magnetic)*sin2alpha
  end subroutine crossterm

  subroutine magnetic_scattering_cmplx(q, xi, sin2alpha, plus_minus, nuclear_parameters,&
    magnetic_parameters, p_nuclear, p_magnetic,&
    Nq, Np_nuc, Np_mag, ff_intensity)
    ! Calculate magnetic scattering
    ! xi: either poleff*flipp or just poleff (or just 1)
    ...
    complex*16 :: f_nuclear, f_magnetic

    interface
      complex*16 function p_nuclear(q, p, Np)
        double precision, intent(in) :: q
        double precision, dimension(Np), intent(in) :: p
        integer, intent(in) :: Np
      end function

      complex*16 function p_magnetic(q, p, Np)
        double precision, intent(in) :: q
        double precision, dimension(Np), intent(in) :: p
        integer, intent(in) :: Np
      end function
    ...
    ff_intensity = abs(f_nuclear)**2 + (abs(f_magnetic)**2 -&
            plus_minus*2d0*xi*abs(f_nuclear*f_magnetic))*sin2alpha
  end subroutine magnetic_scattering_cmplx
end module math
