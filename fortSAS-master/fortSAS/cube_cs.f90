...
contains
  ...
  double precision function p_cube_cs(q, p, Np)
    double precision, intent(in) :: q
    double precision, dimension(Np), intent(in) :: p
    integer, intent(in) :: Np

    double precision :: a, d, SLDcore, SLDshell, SLDmatrix
    ...
    p_cube_cs = ff_amp_shell + ff_amp_core
  end function p_cube_cs

  subroutine oriented_formfactor(&
    q, a, d, SLDcube, SLDshell, SLDmatrix, phi, theta, Nq, ff_intensity)
    double precision, dimension(Nq), intent(in) :: q
    double precision, intent(in) :: a, d, SLDcube, SLDshell, SLDmatrix
    ...
    p = (/a, d, SLDcube, SLDshell, SLDmatrix, phi, theta/)
    !$omp parallel
    !$omp do
    do iq=1, Nq
      ff_intensity(iq) = abs(p_cube_cs(q(iq), p, Np))**2
    end do
    !$omp end do
    !$omp end parallel
  ...
  subroutine formfactor(&
    q, a, d, SLDcube, SLDshell, SLDmatrix, siga,&
    ...
    if (siga > tolerance) then
      do ia=1, N_gh_order
        ...
        do ith=1, N_gl_order
          ...
          do iphi=1, N_gl_order
            ...
            call oriented_formfactor(q, current_a, d,&
      ...
      do ith=1, N_gl_order
        ...
        do iphi=1, N_gl_order
          current_phi =  pi/4d0 * (x_leg(iphi) + 1d0) ! -1 to 1 for 0 to pi/2
          w_phi_i = pi/4d0 * w_leg(iphi)
          call oriented_formfactor(q, a, d,&
            SLDcube, SLDshell, SLDmatrix, current_phi, current_th, Nq, hff)
  ...
  subroutine magnetic_oriented_formfactor(&
    q, a, d, SLDcube, SLDshell, SLDmatrix, phi, theta, &
    mag_SLDcube, mag_SLDshell, mag_SLDmatrix, &
    xi, sin2alpha, plus_or_minus, Nq, ff_out)
    double precision, dimension(Nq), intent(in) :: q
    double precision, intent(in) :: a, d, SLDcube, SLDshell, SLDmatrix, phi, theta
    ...
    contains
      double precision function call_magnetic(q, p, Np)
        ...
        call magnetic_scattering(&
          q, xi, sin2alpha, plus_or_minus, p, p_mag, p_cube_cs, p_cube_cs,&
  ...
  subroutine magnetic_formfactor(&
    q, a, d, SLDcube, SLDshell, SLDmatrix, siga, mag_SLDcube, mag_SLDshell, mag_SLDmatrix,&
    ...
    if (siga > tolerance) then
      ...
      do ith=1, N_gl_order
        ...
        do iphi=1, N_gl_order
          current_phi =  pi/4d0 * (x_leg(iphi) + 1d0) ! -1 to 1 for 0 to pi/2
          w_phi_i = pi/4d0 * w_leg(iphi)
          call magnetic_oriented_formfactor(q, a, d,&
            SLDcube, SLDshell, SLDmatrix, current_phi, current_th,&
            mag_SLDcube, mag_SLDshell, mag_SLDmatrix,&
            xi, sin2alpha, plus_or_minus, Nq, hff)
  ...
  subroutine sld(a, d, SLDcube, SLDshell, SLDmatrix, x, sld_array)
    double precision, intent(in) :: a, d, SLDcube, SLDshell, SLDmatrix
