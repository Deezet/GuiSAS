module superball_css_coupled2
  ...
  contains
    ...
    subroutine formfactor(&
      q, particleSize, dShell, dSurfactant, p_val, SLDsuperball, SLDshell, SLDsurfactant, SLDmatrix,&
      sigParticleSize, sigDShell,&
      x_herm, w_herm, x_leg, w_leg, N_gh_order, N_gl_order, &
      Nq, ff_intensity)
      double precision, dimension(Nq), intent(in) :: q
      double precision, intent(in) :: particleSize, dShell, dSurfactant, p_val
      double precision, intent(in) :: SLDsuperball, SLDshell, SLDsurfactant, SLDmatrix
      double precision, intent(in) :: sigParticleSize, sigDShell
      ...
      if (sigParticleSize > tolerance) then
        do iR=1, N_gh_order
          ...
          w_R_i = w_herm(iR)
          do iD=1, N_gh_order
            current_D =  dShell*dexp(sq2 * x_herm(iD) * sigDShell)
            w_D_i = w_herm(iD)
            do ith=1, N_gl_order
              ...
              do iphi=1, N_gl_order
                ...
                call oriented_formfactor(q, current_particleSize, current_D, dSurfactant, p_val,&
    ...
    subroutine magnetic_formfactor(&
      q, particleSize, dShell, dSurfactant, p_val, SLDsuperball, SLDshell, SLDsurfactant, SLDmatrix, sigParticleSize,&
      sigDShell, mag_SLDsuperball, mag_SLDshell, mag_SLDsurfactant, mag_SLDmatrix, xi, sin2alpha, plus_or_minus, &
      x_herm, w_herm, x_leg, w_leg, N_gh_order, N_gl_order, &
      Nq, ff_intensity)
      double precision, dimension(Nq), intent(in) :: q
      double precision, intent(in) :: particleSize, dShell, dSurfactant, p_val
            ...
            do ith=1, N_gl_order
              ...
              do iphi=1, N_gl_order
                ...
                call magnetic_oriented_formfactor(&
                  q, current_particleSize, current_D, dSurfactant, p_val,&
  ...
  endmodule superball_css_coupled2