module superball
  ...
  contains
    ...
    subroutine oriented_formfactor(&
      q, R, p_val, SLDsuperball, SLDmatrix, alpha, beta, Nq, ff_intensity)
      ...
      !$omp do
      do iq=1, Nq
        ff_intensity(iq) = abs(amplitude_superball_oriented(q(iq), p, Np))**2
      end do
      !$omp end do
      !$omp end parallel
      ...
      if (sigR > tolerance) then
        ...
        do ith=1, N_gl_order
          ...
          do iphi=1, N_gl_order
            current_phi =  pi/4d0 * (x_leg(iphi) + 1d0) ! -1 to 1 for 0 to pi/2
            w_phi_i = pi/4d0 * w_leg(iphi)
            call oriented_formfactor(q, R, p_val,&
              SLDsuperball, SLDmatrix, &
              current_phi, current_th, Nq, hff)
    ...
    subroutine magnetic_oriented_formfactor(&
      q, R, p_val, SLDsuperball, SLDmatrix, alpha, beta, &
      Rmag, mag_SLDsuperball, mag_SLDmatrix, xi, sin2alpha, plus_or_minus,&
      Nq, ff_intensity)
      double precision, dimension(Nq), intent(in) :: q
      double precision, intent(in) :: R, p_val, SLDsuperball, SLDmatrix
      double precision, intent(in) :: alpha, beta
      double precision, intent(in) :: Rmag, mag_SLDsuperball, mag_SLDmatrix
      ...
      double precision function call_magnetic(q, p, Np)
        ...
        p_mag = (/Rmag, p_val, mag_SLDsuperball, mag_SLDmatrix, alpha, beta/)
          ...
          do ith=1, N_gl_order
            ...
            do iphi=1, N_gl_order
              ...
              call magnetic_oriented_formfactor(&
                q, current_R, p_val,&
                SLDsuperball, SLDmatrix, current_phi, current_th, &
                Rmag, mag_SLDsuperball, mag_SLDmatrix, xi, sin2alpha, plus_or_minus, &
                Nq, hff &
              )
