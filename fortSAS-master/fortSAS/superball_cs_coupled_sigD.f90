module superball_cs_coupled_sigd
  use math
  implicit none
  contains
    ...
    double precision function p_superball_cs_coupled(q, p, Np)
      double precision, intent(in) :: q
      double precision, dimension(Np), intent(in) :: p
      integer, intent(in) :: Np

      double precision :: particleSize, R, d, p_val, SLDcore, SLDshell, SLDmatrix
      ...
      p_superball_cs_coupled = ff_amp_shell + ff_amp_core
    end function p_superball_cs_coupled

    subroutine oriented_formfactor(&
      q, particleSize, d, p_val, SLDsuperball, SLDshell, SLDmatrix, alpha, beta, Nq, ff_intensity)
      double precision, dimension(Nq), intent(in) :: q
      double precision, intent(in) :: particleSize, d, p_val
      ...
      p = (/particleSize, d, p_val, SLDsuperball, SLDshell, SLDmatrix, alpha, beta/)

      !$omp parallel
      !$omp do
      do iq=1, Nq
        ff_intensity(iq) = abs(p_superball_cs_coupled(q(iq), p, Np))**2
      end do
      !$omp end do
      !$omp end parallel
    ...
    subroutine formfactor(&
      q, particleSize, d, p_val, SLDsuperball, SLDshell, SLDmatrix, sigParticleSize,&
      sigD, x_herm, w_herm, x_leg, w_leg, N_gh_order, N_gl_order, &
      Nq, ff_intensity)
      double precision, dimension(Nq), intent(in) :: q
      double precision, intent(in) :: particleSize, d, p_val
      double precision, intent(in) :: SLDsuperball, SLDshell, SLDmatrix
      ...
      double precision :: w_R_i, w_D_i, w_phi_i, w_theta_i
      double precision :: intsum_norm
      double precision, dimension(Nq) :: hff, intsum_ff

      integer :: iR, iD, ith, iphi
      double precision :: integrand_prefacs

      intsum_ff = 0d0
      intsum_norm = 0d0
      n_integration_cuts = 1
      if (sigParticleSize > tolerance .OR. sigD > tolerance) then
        do iR=1, N_gh_order
          ! gaussian hermite quadrature
          write(*,'(1a1,i0,A2,$)') char(13), iR * 100/N_gh_order, " %"
          current_ParticleSize = particleSize*dexp(sq2 * x_herm(iR) * sigParticleSize)
          w_R_i = w_herm(iR)
          do iD=1, N_gh_order
            w_D_i = w_herm(iD)
            ...
            do ith=1, N_gl_order
              ...
              do iphi=1, N_gl_order
                ...
                call oriented_formfactor(q, current_ParticleSize, current_D, p_val,&
                  SLDsuperball, SLDshell, SLDmatrix, &
                  current_phi, current_th, Nq, hff)

                integrand_prefacs = w_R_i * w_D_i * w_phi_i * w_theta_i * current_sinth
        ...
        do ith=1, N_gl_order
          ...
          do iphi=1, N_gl_order
            current_phi =  pi/4d0 * (x_leg(iphi) + 1d0) ! -1 to 1 for 0 to pi/2
            w_phi_i = pi/4d0 * w_leg(iphi)
            call oriented_formfactor(q, particleSize, d, p_val,&
              SLDsuperball, SLDshell, SLDmatrix, &
              current_phi, current_th, Nq, hff)
    ...
    subroutine magnetic_oriented_formfactor(&
      q, particleSize, d, p_val, SLDsuperball, SLDshell, SLDmatrix, alpha, beta, &
    ...
    subroutine magnetic_formfactor(&
      q, particleSize, d, p_val, SLDsuperball, SLDshell, SLDmatrix, sigParticleSize,&
      sigD, mag_SLDsuperball, mag_SLDshell, mag_SLDmatrix, xi, sin2alpha, plus_or_minus, &
      ...
      if (sigParticleSize > tolerance) then
            ...
            do ith=1, N_gl_order
              ...
              do iphi=1, N_gl_order
                ...
                call magnetic_oriented_formfactor(&
                  q, current_ParticleSize, current_D, p_val,&
        ...
        do ith=1, N_gl_order
          ...
          do iphi=1, N_gl_order
            ...
            call magnetic_oriented_formfactor(&
              q, current_ParticleSize, d, p_val,&
              SLDsuperball, SLDshell, SLDmatrix, current_phi, current_th, &
              mag_SLDsuperball, mag_SLDshell, mag_SLDmatrix, xi, sin2alpha, plus_or_minus, &
              Nq, hff &
            )
  ...
  endmodule superball_cs_coupled_sigd